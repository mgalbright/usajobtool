...
function SearchForJobs(searchKeyword, seriesCode, city, zipCode){
  ...
  city.length>0 || zipCode.length>0){ 
    ...
    //Use jQuery .ajax() instead of .get(): In the mobile app
    //version, it will be easy to change the headers.
    //Note we can't change headers now since browsers forbid
    //javascript apps from doing so.
    $.ajax({
